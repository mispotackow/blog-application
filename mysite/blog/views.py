...
def post_detail(request, year, month, day, post):
    ...
    if request.method == 'POST':
        ...
        if comment_form.is_valid():
            # Создать объект комментария, но пока не сохранять в базу данных
            new_comment = comment_form.save(commit=False)
            # Назначить текущее сообщение комментарию
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(subject, message, 'potackiw@gmail.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()
